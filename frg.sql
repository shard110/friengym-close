...
ALTER TABLE post ADD replycnt number;
ALTER TABLE post RENAME COLUMN readcount TO viewcnt;

ALTER TABLE post MODIFY viewcnt DEFAULT 0;
ALTER TABLE post MODIFY replycnt DEFAULT 0;
ALTER TABLE post MODIFY poDate DEFAULT CURRENT_TIMESTAMP;
ALTER TABLE Dorder MODIFY DOcount DEFAULT 0;
ALTER TABLE review MODIFY rvDate DEFAULT CURRENT_TIMESTAMP;
...
CREATE OR REPLACE TRIGGER post_before_insert
BEFORE INSERT ON post
FOR EACH ROW
BEGIN
  IF :NEW.ponum IS NULL THEN
    SELECT post_seq.NEXTVAL INTO :NEW.ponum FROM dual;
  END IF;
END;


ALTER TABLE usertbl ADD sessionkey VARCHAR2(50);
ALTER TABLE usertbl ADD sessionlimit TIMESTAMP;

ALTER TABLE reply
MODIFY rpDATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

ALTER TABLE ask
MODIFY aDate TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
...
CREATE OR REPLACE TRIGGER ask_before_insert
BEFORE INSERT ON ask
FOR EACH ROW
BEGIN
  IF :NEW.anum IS NULL THEN
    SELECT ask_seq.NEXTVAL INTO :NEW.anum FROM dual;
  END IF;
END;

ALTER TABLE Manswer
MODIFY mDATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP;

ALTER TABLE product
ADD pDate TIMESTAMP DEFAULT SYSTIMESTAMP;
